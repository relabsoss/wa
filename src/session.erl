...
pass(false, [<<"login">>], Req, State) ->
  case cowboy_req:body_qs(Req) of 
    {ok, ValuesL, Req1} ->
      Values = maps:from_list(ValuesL),
      Mail = maps:get(<<"mail">>, Values, <<"">>),
      Pwd = maps:get(<<"pass">>, Values, <<"">>),
...
pass(false, [<<"reg">>], Req, State) ->
  case cowboy_req:body_qs(Req) of 
    {ok, ValuesL, Req1} ->
      Values = maps:from_list(ValuesL),
      Mail = maps:get(<<"mail">>, Values, <<"">>),
      FName = maps:get(<<"fname">>, Values, <<"">>),
      LName = maps:get(<<"lname">>, Values, <<"">>),
  ...
  case Mail of
    ...
    _ ->
      ...
      case persist:check_user(pgdb, Mail) of
        ...
        false ->
          FName = maps:get(fname, Info, <<"Guest">>),
          LName = maps:get(lname, Info, <<"">>),
      ...
      UpState = case persist:has_social_link(pgdb, maps:get(id, UserInfo)) of
        none ->
          ...
          case persist:get_user(pgdb, Mail) of
            none ->
              case persist:add_user(pgdb, 
                  Mail, 
                  maps:get(fname, UserInfo), 
                  maps:get(lname, UserInfo), 
                  "none") of
                {ok, 1} ->
                  ?SUB(Mail),
