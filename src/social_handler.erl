...
handle(Req, #{action := <<"login">>, options := Options}) ->
  ...
  {ok, Req2} = cowboy_req:reply(302, [
      {<<"location">>, <<URL/binary, $?, (cow_qs:qs(maps:to_list(Params)))/binary>>}
    ], <<>>, Req),
  {ok, Req2, undefined};
handle(Req, #{action := <<"callback">>, options := Options, provider := Provider} = State) ->
  case cowboy_req:qs_val(<<"error">>, Req) of
    {undefined, Req2} ->
      case cowboy_req:qs_val(<<"error_code">>, Req2) of
        {undefined, Req3} ->            
          case check(Options, Req3) of
            {ok, Props, Req4} -> 
                CBL = maps:get(callbacks, Options),
                Req5 = lists:foldl(fun({Mod, Fun}, Req6) -> 
                        Mod:Fun(Provider, Props, Req6)
                    end, Req4, CBL),
                {ok, Req5, State};
            {reply, Req4} -> 
                {ok, Req4, State};
            {error, Req4} -> 
                {ok, fallback(Req4), undefined}
          end;
        {ErrorCode, Req3} ->
          ?ERROR("Error in oauth2 - ~p", [ErrorCode]),
...
check(Options, Req) ->
  case cowboy_req:qs_val(<<"code">>, Req) of
    ...
    {Code, Req1} ->
      case restc:request(post, qs, binary_to_list(maps:get(token_uri, Options)), [200], #{}, #{
            <<"code">> => Code,
            <<"client_id">> => maps:get(client_id, Options),
        ...
        {ok, _Status, _Headers, Body} ->
          case maps:get(error, Body, undefined) of
            undefined ->
              {ok, #{
                  ...
                  email => maps:get(<<"email">>, Body, undefined)
                }, Req1};
            Error ->
              ?ERROR("Error in OAuth access token request ~p", [Error]),
              {error, Req1}
            end;
        Any ->
          ?ERROR("Error in OAuth access token request call ~p", [Any]),
          {error, Req1}
      end
  end.
